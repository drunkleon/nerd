...
.setThumbnail()
.addField(`DLive | Teşekkürler`, `**Selamlar, Ben Yunus Emre (DLive'nin Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`DLive  | Prefix`, `**DLive  Botun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`DLive  | Nasıl Kullanılır?`, `**DLive  botun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`DLive  | Linkler`, `**Sohbet Kanalına .davet Yazmanız Yeterlidir**`)
.setFooter(`DLive  | Gelişmiş Türkçe Bot | 2020`)
.setTimestamp();
...
client.on('message', async msg => {
  ...
  if(msg.content == `<@!763716184267948032>`) return msg.channel.send(`> **DLive | Prefix**\n\n> <a:hypesquad1:763723597863714857> **Sanırım beni etiketlediniz.**\n > <a:hypesquad1:763723597863714857> Buyurun prefix(ön ek)im \`${prefix}\``);
});
...
.setDescription(`<a:hypesquad1:763723597863714857> <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("DLive Bot | Log Sistemi")
...
if (!modlogkanal) return;
  ...
  .setColor("#ffd100")
    ...
    .setDescription(`<a:hypesquad1:763723597863714857> **Üye Sunucudan Yasaklandı!** \n<@!${message.user.id}>, ${message.user.tag}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setDescription(`<a:hypesquad1:763723597863714857> ${channel.name} **Adlı Metin Kanalı Oluşturuldu!**`)
                .setFooter(`DLive Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
...
.setTitle("SES KANALI OLUŞTURULDU")
                .setDescription(`<a:hypesquad1:763723597863714857> ${channel.name} **Adlı Ses Kanalı Oluşturuldu!**`)
                .setFooter(`DLive Bot | Log Sistemi Kanal ID: ${channel.id}`)
    ...
    if (küfür.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`<a:hypesquad1:763723597863714857> **Hey ${message.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`<a:hypesquad1:763723597863714857> **Hey ${newMessage.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
    ...
    if (reklamengel.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`<a:hypesquad1:763723597863714857> **Hey ${message.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
    ...
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'sa' || msg.content.toLowerCase() == 's.a' || msg.content.toLowerCase() == 'selamun aleyküm' || msg.content.toLowerCase() == 'sea' || msg.content.toLowerCase() == 's.a.' || msg.content.toLowerCase() == 'selam' || msg.content.toLowerCase() == 'slm') {
          try {
                  ...
                  return msg.reply('**<a:hypesquad1:763723597863714857> Aleyküm Selam, Hoşgeldin.** ')
          } catch(err) {
            console.log(err);
          }
      }
...
client.on("guildMemberAdd", async member => {
  ...
  if (!mesaj) {
    client.channels.cache.get(kanal).send("a:hypesquad1:763723597863714857> `" + member.user.username + "`** Hoş Geldin! Otomatik Rolün Verildi Seninle Beraber** `" + member.guild.memberCount + "` **Kişiyiz!**");
